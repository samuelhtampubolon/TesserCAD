...
**That cleanup does not stay done by itself.** A pull request body is stamped
after submission with a `Generated by Claude Code` footer whose href carries
the live session id, whatever the author wrote. PR #22 was opened without one
and had one a moment later. So the rule above, no session link in a pull
request description, cannot be satisfied by writing the body correctly: it
has to be read back after opening and the href repointed at
`https://claude.ai/code`. The disclosure line stays; only the id goes.

Auditing every body at once is one query, and should return nothing:
